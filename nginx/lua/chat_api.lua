-- nginx/lua/chat_api.lua - Full updated with true chunk streaming and all endpoints
...
local function check_rate_limit(red, username)
    ...
    local current_count = tonumber(red:get(count_key) or 0)
    if current_count >= RATE_LIMIT_MESSAGES_PER_MINUTE * 60 then
        send_json(429, { error = "Rate limit exceeded" })
    end
    red:incr(count_key)
...
local function handle_chat_stream()
    ...
    local final_response, err = call_ollama_streaming(messages, options, function(chunk)
        ...
        send_sse_chunk({ content = chunk.content, done = chunk.done })
    end)
