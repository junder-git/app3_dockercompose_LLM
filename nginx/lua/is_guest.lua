...
return {
    ...
    -- Core functions (for is_who.lua to use)
...
local function handle_create_guest_session()
    ...
    if not session_data then
        ...
        -- Get current stats to show accurate info
        local stats, _ = server.get_guest_stats()
        local max_guests = (stats and stats.max_sessions) or 2
...
local function handle_guest_stats()
    ...
    -- Public version (no sensitive data) - uses actual server configuration
    send_json(200, {
        success = true,
        stats = {
            active_sessions = stats.active_sessions,
        ...
        info = {
            ...
            max_concurrent_guests = stats.max_sessions  -- Use actual value from server
        },
        availability = {
            can_create_session = stats.available_slots > 0,
            estimated_wait_time = stats.available_slots == 0 and "1-30 minutes" or "immediate"
